...
r= re.compile('^[a-z][a-z0-9]'     )##ID 정규식
while True :
    str = input("ID입력")
...
r= re.compile( '[0-9][^alpha]'    )##Password 정규식
while True :
...
r= re.compile( '(\d)-(\d{3-4})-(\d{4})'    )##일반전화번호 정규식
while True :
    str = input("일반전화번호입력")
    if r.search(str) == None:
         print("잘못입력")
...
r= re.compile(  '(\d{2})-(\d{4})-(\d{4})'   )##핸드폰 번호 정규식
while True :
    str = input("핸드폰번호입력")
    if r.search(str) == None:
         print("잘못입력")
